...
class TestCORSOrigins:
    ...
    def test_cors_origins_includes_new_preview_url(self):
        """Test that CORS origins includes the new preview URL"""
        test_settings = Settings()
    ...
    def test_cors_origins_includes_production_url(self):
        """Test that CORS origins includes the production URL"""
        test_settings = Settings()
        
        assert "https://fix-it-6.emergent.host" in test_settings.cors_origins
        
    def test_cors_origins_excludes_old_preview_url(self):
        """Test that CORS origins does not include the old preview URL"""
        test_settings = Settings()
        
        # The old URL should not be present if it was replaced
        if "https://fix-it-6.preview.emergentagent.com" in test_settings.cors_origins:
            pytest.fail("Old preview URL should have been replaced")
    
    @patch.dict(os.environ, {
        'CORS_ORIGINS': 'https://custom1.example.com,https://custom2.example.com'
    })
    def test_cors_origins_custom_from_env(self):
        """Test CORS origins can be fully customized via environment variable"""
        test_settings = Settings()
        
        assert "https://custom1.example.com" in test_settings.cors_origins
        assert "https://custom2.example.com" in test_settings.cors_origins
    ...
    def test_cors_origins_format_validation(self):
        """Test that CORS origins are properly formatted URLs"""
        test_settings = Settings()
        
        for origin in test_settings.cors_origins:
            assert origin.startswith("http://") or origin.startswith("https://")
            assert len(origin) > 10  # Reasonable URL length
    ...
    def test_cors_origins_no_duplicates(self):
        ...
        unique_origins = set(test_settings.cors_origins)
        assert len(unique_origins) == len(test_settings.cors_origins)
        
    @patch.dict(os.environ, {
        'CORS_ORIGINS': 'http://localhost:3000,http://localhost:3001,https://staging.example.com,https://prod.example.com'
    })
    def test_cors_origins_multiple_environments(self):
        """Test CORS configuration with multiple environment URLs"""
        test_settings = Settings()
    ...
    def test_cors_origins_type_safety(self):
        """Test that cors_origins is always a list of strings"""
        test_settings = Settings()
    ...
    def test_cors_origins_empty_string_handling(self):
        ...
        # Should result in a list with one empty string
        assert isinstance(test_settings.cors_origins, list)
        # This is expected behavior - empty env var creates ['']
...
class TestConfigurationEdgeCases:
    ...
    def test_settings_immutability_attempt(self):
        """Test that settings can be modified (pydantic allows it)"""
        test_settings = Settings()
        original_db_name = test_settings.db_name
        
        # Pydantic BaseSettings allows attribute modification
        test_settings.db_name = "modified_db"
        assert test_settings.db_name == "modified_db"
        
        # Create new instance to verify defaults unchanged
        new_settings = Settings()
        assert new_settings.db_name == original_db_name
    ...
    def test_multiple_settings_instances_independence(self):
        ...
        settings1.db_name = "db1"
        settings2.db_name = "db2"
        
        assert settings1.db_name != settings2.db_name
        
    @patch.dict(os.environ, {'MONGO_URL': 'mongodb://user:pass@host:27017'})
    def test_mongo_url_with_credentials(self):
        """Test MongoDB URL with authentication credentials"""
        test_settings = Settings()
        
        assert "user:pass" in test_settings.mongo_url
        assert "@host:27017" in test_settings.mongo_url
        
    def test_file_upload_constraints(self):
        """Test file upload size and type constraints are reasonable"""
        test_settings = Settings()
        
        # Max file size should be between 1MB and 100MB
        assert 1024 * 1024 <= test_settings.max_file_size <= 100 * 1024 * 1024
        
        # Should have at least image types
        image_types = [ft for ft in test_settings.allowed_file_types if ft.startswith("image/")]
        assert len(image_types) >= 2
    ...
    def test_empty_api_keys_handling(self):
        """Test handling of empty API keys"""
        test_settings = Settings()
        
        assert test_settings.openai_api_key == ''
        assert test_settings.sendgrid_api_key == ''
        assert test_settings.stripe_api_key == ''
        
    def test_jwt_algorithm_is_valid(self):
        """Test that JWT algorithm is a recognized value"""
        test_settings = Settings()
        
        valid_algorithms = ['HS256', 'HS384', 'HS512', 'RS256', 'RS384', 'RS512']
        assert test_settings.jwt_algorithm in valid_algorithms
        
    def test_default_ai_model_format(self):
        """Test that default AI model follows expected format"""
        test_settings = Settings()
        
        assert isinstance(test_settings.default_ai_model, str)
        assert len(test_settings.default_ai_model) > 0
        # Common model patterns: gpt-*, claude-*, gemini-*
        assert any(prefix in test_settings.default_ai_model.lower() 
                  for prefix in ['gpt', 'claude', 'gemini', 'llama'])
